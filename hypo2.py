...
def jointHypoVel(par, grid, data, rcv, Vinit, hinit, caldata=np.array([]),
    ...
    sc = np.zeros(nsta)
    hyp0 = hinit.copy()
    nnodes = grid.get_number_of_nodes()
    ...
    if par.invert_vel:
        ...
        deltam = np.zeros(nnodes+nsta)
        if par.constr_sc and par.use_sc:
            u1 = np.ones(nnodes+nsta)
            u1[:nnodes] = 0.0

            i = np.arange(nnodes, nnodes+nsta)
            j = np.arange(nnodes, nnodes+nsta)
            nn = i.size
            i = np.kron(i, np.ones((nn,)))
            j = np.kron(np.ones((nn,)), j)
            u1Tu1 = sp.csr_matrix((np.ones((i.size,)), (i, j)),
                                  shape=(nnodes+nsta, nnodes+nsta))
        else:
            u1 = np.zeros(nnodes+nsta)
            u1Tu1 = sp.csr_matrix((nnodes+nsta, nnodes+nsta))
    ...
    for it in np.arange(par.maxit):
        ...
        if par.invert_vel:
            ...
            for n in np.arange(nnodes):
                if V[n] < par.Vpmin:
                    P[n] = par.PAp * (par.Vpmin-V[n])
                    dP[n, n] = -par.PAp
                elif V[n] > par.Vpmax:
                    P[n] = par.PAp * (V[n]-par.Vpmax)
                    dP[n, n] = par.PAp
                else:
                    P[n] = 0.0
                    dP[n, n] = 0.0
            ...
            if nev > 0:
                ...
                for ne in np.arange(nev):
                    ...
                    for i in indr:
                        hyp[i, :] = hyp0[indh[0], :]
                tcalc, rays, Mev = grid.raytrace(hyp, rcv_data, s,
                                                 return_rays=True,
                                                 compute_M=True)
                s0 = grid.get_s0(hyp)
            else:
            ...
            if ncal > 0:
                tcalc_cal, Mcal = grid.raytrace(hcal, rcv_cal, s,
                                                compute_M=True)
            else:
                tcalc_cal = np.array([])
...
def jointHypoVel_c(par, grid, data, rcv, Vinit, hinit, caldata=np.array([]),
                   Vpts=np.array([])):
    """
    Joint hypocenter-velocity inversion on a regular grid
    ...
    if par.invert_vel:
        ...
        P = np.ones(nslowness)
        dP = sp.csr_matrix((np.ones(nslowness), (np.arange(nslowness, dtype=np.int64),
                                     np.arange(nslowness, dtype=np.int64))), shape=(nslowness, nslowness))
    ...
    for it in np.arange(par.maxit):
        ...
        if par.invert_vel:
            ...
            dP1 = sp.hstack((dP, sp.csr_matrix(np.zeros((nslowness, nsta))))).tocsr()  # dP prime
...
def jointHypoVelPS(par, grid, data, rcv, Vinit, hinit, caldata=np.array([]),
    ...
    if grid.nthreads > 1:
        ...
        grid_s = Grid3d(grid.x, grid.y, grid.z, grid.nthreads, cell_slowness=False)
    else:
        grid_s = grid
    ...
    if np.isscalar(Vinit[0]):
        Vp = Vinit[0] + np.zeros(nnodes)
        s_p = np.ones(nnodes)/Vinit[0]
    else:
        Vp = Vinit[0]
        s_p = 1./Vinit[0]
    if np.isscalar(Vinit[1]):
        Vs = Vinit[1] + np.zeros(nnodes)
    ...
    if par.invert_vel:
        ...
        deltam = np.zeros(2*nnodes+2*nsta)

        if par.constr_sc and par.use_sc:
            u1 = np.ones(2*nnodes+2*nsta)
            u1[:2*nnodes] = 0.0
            u1[(2*nnodes+nsta):] = 0.0

            i = np.arange(2*nnodes, 2*nnodes+nsta)
            j = np.arange(2*nnodes, 2*nnodes+nsta)
            nn = i.size
            i = np.kron(i, np.ones((nn,)))
            j = np.kron(np.ones((nn,)), j)
            u1Tu1 = sp.csr_matrix((np.ones((i.size,)), (i, j)),
                                  shape=(2*nnodes+2*nsta, 2*nnodes+2*nsta))
        else:
            u1 = np.zeros(2*nnodes+2*nsta)
            u1Tu1 = sp.csr_matrix((2*nnodes+2*nsta, 2*nnodes+2*nsta))
    ...
    for it in np.arange(par.maxit):
        ...
        if par.invert_vel:
            ...
            for n in np.arange(nnodes, 2*nnodes):
                if par.invert_VsVp:
                    if VsVp[n-nnodes] < VsVpmin:
                        P[n] = par.PAs * (VsVpmin-VsVp[n-nnodes])
                        dP[n, n] = -par.PAs
                    elif VsVp[n-nnodes] > VsVpmax:
                        P[n] = par.PAs * (VsVp[n-nnodes]-VsVpmax)
                        dP[n, n] = par.PAs
                    else:
                        P[n] = 0.0
                        dP[n, n] = 0.0
                else:
                    if Vs[n-nnodes] < par.Vsmin:
                        P[n] = par.PAs * (par.Vsmin-Vs[n-nnodes])
                        dP[n, n] = -par.PAs
                    elif Vs[n-nnodes] > par.Vsmax:
                        P[n] = par.PAs * (Vs[n-nnodes]-par.Vsmax)
                        dP[n, n] = par.PAs
                    else:
                        P[n] = 0.0
                        dP[n, n] = 0.0
            ...
            if par.verbose:
                npel = np.sum(P[:nnodes] != 0.0)
                if npel > 0:
                    print('    P-wave penalties applied at {0:d} nodes'.format(npel))
                npel = np.sum(P[nnodes:2*nnodes] != 0.0)
                if npel > 0:
            ...
            if nev > 0:
                ...
                tcalcp, raysp, Mevp = grid.raytrace(hyp, rcv_datap, s_p,
                                                    return_rays=True,
                                                    compute_M=True)
                s0p = grid.get_s0(hyp)
                ...
                tcalcs, rayss, Mevs = grid_s.raytrace(hyp, rcv_datas, s_s,
                                                      return_rays=True,
                                                      compute_M=True)
                s0s = grid.get_s0(hyp)
                ...
                for ne in np.arange(nev):
                    ...
                    if par.invert_VsVp:
                        # Block 1991, p. 45
                        tmp1 = Ms.multiply(np.tile(VsVp, (Ms.shape[0], 1)))
                        tmp2 = Ms.multiply(np.tile(Vp, (Ms.shape[0], 1)))
            ...
            if ncal > 0:
                tcalcp_cal, Mp_cal = grid.raytrace(hcalp, rcv_calp, s_p,
                                                   compute_M=True)
                if nttcals > 0:
                    tcalcs_cal, Ms_cal = grid_s.raytrace(hcals, rcv_cals, s_s,
                                                         compute_M=True)
            ...
            s_p = 1./Vp
            s_s = 1./Vs
            sc_p += deltam[2*nnodes:2*nnodes+nsta]
            sc_s += deltam[2*nnodes+nsta:]
            ...
            if par.save_V:
                ...
                if par.invert_VsVp:
                    if 'vtk' in sys.modules:
                        grid.to_vtk({'VsVp': VsVp}, 'VsVp{0:02d}'.format(it+1))
                if 'vtk' in sys.modules:
...
def jointHypoVelPS_c(par, grid, data, rcv, Vinit, hinit, caldata=np.array([]),
                     Vpts=np.array([])):
    """
    Joint hypocenter-velocity inversion on a regular grid
    ...
    for n in range(data.shape[0]):
        try:
            ...
            raise ValueError('Event '+str(data[n, 0])+' not fount in hinit')
    ...
    if par.invert_vel:
        ...
        dP = sp.csr_matrix((np.ones(2*nslowness), (np.arange(2*nslowness,
                                                          dtype=np.int64),
                            np.arange(2*nslowness, dtype=np.int64))),
                           shape=(2*nslowness, 2*nslowness))
        ...
        if par.constr_sc and par.use_sc:
            ...
            j = np.kron(np.ones((nn,)), j)
            u1Tu1 = sp.csr_matrix((np.ones((i.size,)), (i, j)),
                                  shape=(2*nslowness+2*nsta, 2*nslowness+2*nsta))
        else:
    ...
    for it in np.arange(par.maxit):
        ...
        if par.invert_vel:
            ...
            dP1 = sp.hstack((dP, sp.csr_matrix((2*nslowness, 2*nsta)))).tocsr()  # dP prime
            ...
            if par.save_V:
                ...
                if par.invert_VsVp:
                    if 'vtk' in sys.modules:
                        grid.to_vtk({'VsVp': SsSp}, 'VsVp{0:02d}'.format(it+1))
                if 'vtk' in sys.modules:
                    grid.to_vtk({'Vs': Vs}, 'Vs{0:02d}'.format(it+1))
...
# %% testPS
    if testPS:
        ...
        slowness_s = 1./Vs + np.zeros(g.get_number_of_nodes())
        ...
        caldata_s = np.column_stack((src_cal[:, 0], tcal_s, ircv_cal, src_cal[:, 2:],
...
# %% testCps
    if testCps:
        ...
        plt.figure(figsize=(10, 4))
        plt.subplot(121)
        plt.plot(err_x, 'o', label=r'jhv - $\|\|\Delta x\|\|$ = {0:6.5f}'.format(np.linalg.norm(err_x)))
        plt.plot(err_xc, 'r*', label=r'cst v - $\|\|\Delta x\|\|$ = {0:6.5f}'.format(np.linalg.norm(err_xc)))
        plt.ylabel(r'$\Delta x$')
        plt.xlabel('Event ID')
        plt.legend()
        plt.subplot(122)
        plt.plot(np.abs(err_t), 'o', label=r'jhv - $\|\|\Delta t\|\|$ = {0:6.5f}'.format(np.linalg.norm(err_t)))
        plt.plot(np.abs(err_tc), 'r*', label=r'cst v - $\|\|\Delta t\|\|$ = {0:6.5f}'.format(np.linalg.norm(err_tc)))
        plt.ylabel(r'$\Delta t$')
        plt.xlabel('Event ID')
        plt.tight_layout()
        plt.legend()
