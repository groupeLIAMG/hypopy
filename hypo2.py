...
def jointHypoVelPS(par, grid, data, rcv, Vinit, hinit, caldata=np.array([]),
    ...
    for it in np.arange(par.maxit):
        ...
        if par.invert_vel:
            ...
            if dmax > dVp_max:
                if par.verbose:
                    print('  Scaling P slowness perturbations by {0:e}'.format(dVp_max/dmax))
                deltam[:nslowness] = deltam[:nslowness] * dVp_max / dmax
            dmax = np.max(np.abs(deltam[nslowness:2*nslowness]))
            if dmax > dVs_max:
                if par.verbose:
                    print('  Scaling S slowness perturbations by {0:e}'.format(dVs_max/dmax))
        ...
        for itt in range(par.maxit_hypo):
            ...
            try:
                ...
                if 'going outside grid' in str(rte):
                    print('  Problem while computing P-wave traveltimes, resetting and exiting')
                    hyp0[indh, :] = hyp_save
                    return hyp_save, indh
                else:
                    raise rte
            ...
            try:
                ...
                if 'going outside grid' in str(rte):
                    print('  Problem while computing S-wave traveltimes, resetting and exiting')
                    hyp0[indh, :] = hyp_save
                    return hyp_save, indh
                else:
                    raise rte
